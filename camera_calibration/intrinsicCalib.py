...
parser.add_argument('-fw','--FRAME_WIDTH', default=800, type=int, help='Camera Frame Width')
parser.add_argument('-fh','--FRAME_HEIGHT', default=600, type=int, help='Camera Frame Height')
parser.add_argument('-bw','--BORAD_WIDTH', default=7, type=int, help='Chess Board Width (corners number)')
parser.add_argument('-bh','--BORAD_HEIGHT', default=10, type=int, help='Chess Board Height (corners number)')
...
def main():
    ...
    # np.save('camera_{}_D.npy'.format(args.CAMERA_ID),result.dist_coeff.tolist())
     # Save the calibration data to an .npz file
    save_file = f'camera_{args.CAMERA_ID}_calibration.npz'
    np.savez(save_file, 
             camera_matrix=result.camera_mat, 
             dist=result.dist_coeff, 
             rvecs=result.rvecs, 
             tvecs=result.tvecs)
